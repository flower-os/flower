...
pub fn write(port: &Port<u8>, value: u8) -> ps2::Result<()> {
    ...
    if can_write() {
        ...
        Err(ps2::Error::WriteUnavailable)
    }
}
...
pub fn write_blocking(port: &Port<u8>, value: u8) {
...
pub fn read_blocking(port: &Port<u8>) -> Option<u8> {
...
pub fn can_read_keyboard() -> bool {
    ...
    status.contains(StatusFlags::OUTPUT_FULL) && !status.contains(StatusFlags::PORT_2_FULL)
}
